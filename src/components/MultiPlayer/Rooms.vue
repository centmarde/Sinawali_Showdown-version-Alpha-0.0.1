<template>
    <v-card class="pa-4">
        ...
        <v-card-text>
            <v-text-field v-model="roomName" label="Room Name" required placeholder="Enter room name"></v-text-field>
        ...
        <v-row dense>
            <v-col v-for="room in rooms" :key="room.id" cols="6">
                <v-card outlined class="pa-4">
                    ...
                    <v-card-actions>
                        <v-btn color="primary" @click.stop="enterRoom(room)" :disabled="!!room.isFull">
                            BATTLE
                        </v-btn>
                        <v-btn v-if="room.isFull" color="green" @click.stop="startBattle(room)">
                            On Battle!
                        </v-btn>
                    </v-card-actions>
                </v-card>
